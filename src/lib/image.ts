...
export function optimizeImage(urlOrObj: string | { url?: string } | null): string | null {
  return transformImage(urlOrObj)
}

export function transformImage(urlOrObj: string | { url?: string } | null, opts: TransformOpts = {}): string | null {
  const url = typeof urlOrObj === 'string' ? urlOrObj : urlOrObj?.url ?? urlOrObj
  if (!url || typeof url !== 'string') return urlOrObj as null
